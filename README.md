# Hi there, I'm Soundarya Dube! 👋

Computer Science student at Vellore Institute of Technology, Bhopal passionate about building scalable AI systems and intelligent automation.

Currently working at Qumin.ai, contributing to AI automation, software testing workflows, and scalable QA systems across web and mobile platforms.

My work focuses on machine learning, multimodal AI, automation, and data-driven systems. I enjoy solving problems where large models interact with real-world data, interfaces, and intelligent workflows.

## 🚀 Experience at Qumin

- Worked on AI-driven automation and QA workflows
- Performed functional, regression, smoke, UI, and cross-browser testing
- Contributed to testing and validation across web and mobile applications
- Collaborated on automation systems and scalable testing processes
- Participated in release-day testing, debugging, and deployment validation

## 🚀 Recent Work

- Built asynchronous Python pipelines for large-scale web data collection
- Developed LLM-powered structured data extraction systems using Gemini
- Worked on evaluation frameworks for Vision-Language Models (VLMs)
- Created spatial consensus systems for UI element detection and automation

## 🛠️ Technologies & Tools

**Languages & Frameworks**  
Python • PyTorch • TensorFlow • FastAPI • OpenCV • Pandas • Asyncio

**Testing & Automation**  
Functional Testing • Regression Testing • Smoke Testing • UI Testing • Cross-Browser Testing

**Areas of Interest**  
LLM Systems • Multimodal AI • AI Automation • Computer Vision • Data Pipelines

## 🌱 Currently Exploring

- Advanced LLM systems
- Multimodal model architectures
- AI-powered automation workflows
- Intelligent UI understanding systems


# 💻 Featured YouTube Videos

<!-- BEGIN YOUTUBE-CARDS -->
[![Life of an AI Automation Engineer at Qumin](https://ytcards.demolab.com/?id=KgYlrpn2zAU&title=Life+of+an+AI+Automation+Engineer+at+Qumin&lang=en&timestamp=1778544000&background_color=%230d1117&title_color=%23ffffff&stats_color=%23dedede&max_title_lines=1&width=250&border_radius=5&duration=90)](https://youtu.be/KgYlrpn2zAU)
[![Can You Build Production-Ready AI Systems?](https://ytcards.demolab.com/?id=0A1BtN84IdY&title=Can+You+Build+Production-Ready+AI+Systems&lang=en&timestamp=1778716800&background_color=%230d1117&title_color=%23ffffff&stats_color=%23dedede&max_title_lines=1&width=250&border_radius=5&duration=39)](https://youtu.be/0A1BtN84IdY)
<!-- END YOUTUBE-CARDS -->


⭐ Always open to collaborating on AI, automation, scalable ML systems, and intelligent testing workflows.
